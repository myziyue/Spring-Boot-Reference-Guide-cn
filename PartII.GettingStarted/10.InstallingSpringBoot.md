...
spring-boot-starter-parent是一种使用Spring Boot比较好的方式，但是它并不是所有情况都适用。有时，你可能不喜欢我们的默认设置，你可以继承一个不同的父POM。请看“在不使用父POM情况下适用Spring Boot”章节提供的可选择的替代方案，它适用了`import`作用域。
